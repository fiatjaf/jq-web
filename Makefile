...
jq.js: jq/jq.o pre.js post.js
	cd jq && \
	  emcc -O3 -s TOTAL_MEMORY=32MB -s MODULARIZE=1 --memory-init-file 1 --pre-js ../pre.js --post-js ../post.js jq.o -o ../jq.js
...
jq.bundle.js: jq/jq.o pre.js post.js
	cd jq && \
	  emcc -O3 -s ALLOW_MEMORY_GROWTH=1 -s MODULARIZE=1 --memory-init-file 0 --pre-js ../pre.js --post-js ../post.js jq.o -o ../jq.bundle.js
...
jq.wasm.js: jq/jq.o pre.js post.js
	cd jq && \
	  emcc -O3 -s ALLOW_MEMORY_GROWTH=1 -s MODULARIZE=1 -s WASM=1 --pre-js ../pre.js --post-js ../post.js jq.o -o ../jq.wasm.js
